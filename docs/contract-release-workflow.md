...
- Release: `https://github.com/MB3R-Lab/Bering/releases/tag/v1.0.0`
- Schema contract pins consumed by Sheaft remain explicit in `internal/modelcontract/contract.go` and currently include only `1.3.0`.
...
`schema/index.json` tracks the latest published Bering contract line. The current Sheaft v1 line supports only the exact published `1.3.0` model and snapshot schema URLs plus the checked-in versioned schema mirrors.
