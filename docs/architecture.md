...
2. `internal/artifact` validates the declared contract against a supported whitelist and adapts the artifact into a normalized internal model plus typed `1.2.0` discovery metadata when present.
3. `internal/analyzer` resolves predicates, optional overlays, journeys, fault profiles, baselines, and profile configuration.
4. `internal/simulation` runs deterministic Monte Carlo analysis for one or more profiles, preserving `1.0.0` baseline semantics while enabling `1.2.0` path-aware diagnostics.
