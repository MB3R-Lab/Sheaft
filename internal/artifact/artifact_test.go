...
func TestLoad_CheckedInSnapshotV130Sample(t *testing.T) {
	t.Parallel()
	...
	path := filepath.Join(filepath.Dir(thisFile), "..", "..", "examples", "outputs", "snapshot-v1.3.0.sample.json")

	loaded, err := Load(path)
	if err != nil {
		t.Fatalf("Load checked-in snapshot v1.3.0 sample failed: %v", err)
	}
	if loaded.Metadata.Kind != modelcontract.KindSnapshot {
		t.Fatalf("expected snapshot kind, got %s", loaded.Metadata.Kind)
	}
	if loaded.Metadata.Contract.Version != modelcontract.BeringSnapshotV130Version {
		t.Fatalf("unexpected snapshot contract version: %+v", loaded.Metadata.Contract)
	}
	if loaded.Snapshot == nil {
		t.Fatal("expected typed snapshot metadata for v1.3.0 artifact")
	}
	if loaded.Model.Edges[0].ID == "" {
		t.Fatal("expected edge ids to be preserved for v1.3.0 model")
	}
}

func TestLoad_CheckedInModelV130Sample(t *testing.T) {
	t.Parallel()
	...
	path := filepath.Join(filepath.Dir(thisFile), "..", "..", "examples", "outputs", "model-v1.3.0.sample.json")

	loaded, err := Load(path)
	if err != nil {
		t.Fatalf("Load checked-in model v1.3.0 sample failed: %v", err)
	}
	if loaded.Metadata.Kind != modelcontract.KindModel {
		t.Fatalf("expected model kind, got %s", loaded.Metadata.Kind)
	}
	if loaded.Metadata.Contract.Version != modelcontract.BeringModelV130Version {
