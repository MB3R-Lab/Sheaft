...
func ValidateV1ReleaseDocs(root string) error {
	...
	requiredPaths := []string{
		...
		"examples/outputs/model-v1.1.0.sample.json",
		"examples/outputs/snapshot-v1.1.0.sample.json",
...
func validateV1CompatibilityDocs(root string) error {
	...
	for _, contract := range []modelcontract.SupportedContract{
		...
		{
			Name:    modelcontract.BeringSnapshotV110Name,
			Version: modelcontract.BeringSnapshotV110Version,
			URI:     modelcontract.BeringSnapshotV110URI,
			Digest:  modelcontract.BeringSnapshotV110Digest,
		},
	} {
		if err := requireContains(DefaultV1SemanticsDocPath, v1Doc, []string{
			fmt.Sprintf("%s@%s", contract.Name, contract.Version),
...
func validateV1Examples(root string) error {
	...
	if err := readReleaseJSON(root, "examples/outputs/model-v1.1.0.sample.json", &sample); err != nil {
		return err
	}
	if err := requireSchemaRef("model-v1.1.0 sample", sample.Metadata.Schema, modelcontract.ExpectedModelV110Ref()); err != nil {
		return err
	}
	if err := requireV1ExampleFeatures("model-v1.1.0 sample", sample.Services, sample.Edges, sample.Endpoints); err != nil {
		return err
	}

	var snapshot v1SampleSnapshot
	if err := readReleaseJSON(root, "examples/outputs/snapshot-v1.1.0.sample.json", &snapshot); err != nil {
		return err
	}
	if err := requireSchemaRef("snapshot-v1.1.0 sample", snapshot.Metadata.Schema, modelcontract.ExpectedSnapshotV110Ref()); err != nil {
		return err
	}
	if err := requireSchemaRef("snapshot-v1.1.0 embedded model", snapshot.Model.Metadata.Schema, modelcontract.ExpectedModelV110Ref()); err != nil {
		return err
	}
	if err := requireV1ExampleFeatures("snapshot-v1.1.0 embedded model", snapshot.Model.Services, snapshot.Model.Edges, snapshot.Model.Endpoints); err != nil {
		return err
	}
	if err := requireV1ExampleFeatures("snapshot-v1.1.0 discovery", snapshot.Discovery.Services, snapshot.Discovery.Edges, snapshot.Discovery.Endpoints); err != nil {
		return err
	}
	return nil
}
